...
class autonomous_flight():
    ...
    def __init__(self):        
        ...
        self.landing_station_one_waypoints = [[[3.65, 1.10, 1.5, 90], [3.65, 4.25, 1.5, 90],
        ...
        self.landing_station_three_waypoints = [[[3.65, 1.10, 1.5, 90], [3.65, 4.25, 1.5, 90],
    ...
    def drone_takeoff(self, alt=20.0):
    ...
    def GPS2Vision_test(self):
        ...
        #Only used for plotting pos of the drone for different states 
        self.write_data_log_for_gps2vision_state = 0
    ...
    def hold_aruco_pose_test(self):
        ...
        if gps2vision_complete:
            ...
            setpoint = [3.65, -1, 2.50, 0, 0, 90] #x, y, z, roll, pitch, yaw (GPS2Vision board)
            #setpoint = [0.40, 7.10, 1.5, 0, 0, 90] #x, y, z, roll, pitch, yaw (Landing board one)
            #setpoint = [3.65, 7.10, 1.5, 0, 0, 90] #x, y, z, roll, pitch, yaw (Landing board two)
            #setpoint = [7.00, 7.10, 1.5, 0 ,0, 90] #x, y, z, roll, pitch, yaw (Landing board three)
    ...
    def vision_navigation_test(self):
        ...
        self.GPS_navigation(waypoints_xyzYaw=[[0, 0, -1.0, 0]])
        gps2vision_complete = self.GPS2Vision()
    ...
    def return_to_landing_station_one(self):
        ...
        self.drone_takeoff(alt =  20.0)
        self.set_state('return_to_landing_station_one')
        rospy.loginfo('Autonomous_flight: Returning to landing station one!')

        self.GPS_navigation(waypoints_xyzYaw=[[6, 0, -0.5, 0]])
        gps2vision_complete = self.GPS2Vision()
        if gps2vision_complete:
    ...
    def navigate_to_marker_board(self):
        ...
        while (self.dis_to_GPS2Vision_marker > min_dis) and not self.aruco_marker_pose_stable:
            ...
            if self.write_data_log_for_gps2vision:
                self.log_data.write_gps2vision_data(self.ground_truth, self.write_data_log_for_gps2vision_state)
    ...
    def vision_landing_navigation(self, landing_station, waypoint_check_pose_error = 0.1):
        ...
        if landing_station == 3:
            waypoints_xyzYaw = [[0, 0, 1.5, 90], [0, 0, 0.2, 90]]
    ...
    def vision_takeoff_navigation(self, landing_station):
        ...
        self.flight_mode.set_arm(True,10)
        self.flight_mode.set_mode('OFFBOARD',5)

        if landing_station == 3:
            waypoints_xyzYaw = [[0, 0, 1.5, 90]]
        elif landing_station == 4:
    ...
    def validate_marker_board_detections(self):
        threshold = 0.4 #80% marker board detections to yield true 
        detections = 0.0
